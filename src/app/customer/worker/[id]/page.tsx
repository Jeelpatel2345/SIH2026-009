        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black text-teal-800">₹{calculatedTotal}</span>
              <span className="text-xs text-slate-400 font-medium">+ taxes & visit</span>
            </div>
          </div>

          <Link
            href={`/customer/booking/${worker.id}?hours=${selectedHours}&amount=${calculatedTotal}`}
            className="bg-teal-700 hover:bg-teal-800 text-white px-6 sm:px-10 py-3.5 rounded-2xl font-bold text-sm sm:text-base flex items-center gap-2 shadow-lg shadow-teal-900/20 transition"
          >
